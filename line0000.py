...
axes[1].plot(np.exp(sampler.chain[:, :, 2]).T, color="k", alpha=0.4)
...
pl.figure()
for T_ls, lnf in samples[np.random.randint(len(samples), size=100)]:
...
T_mcmc, f_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
